...
class CategorizedAccountNumbersDeserializer : JsonDeserializer<List<CategorizedAccountNumbers>> {
    override fun deserialize(json: JsonElement?, typeOfT: Type?, context: JsonDeserializationContext): List<CategorizedAccountNumbers> {
        val items = arrayListOf<CategorizedAccountNumbers>()
        json?.asJsonObject?.getAsJsonArray("items")?.forEach { items.add(context.deserialize(it, CategorizedAccountNumbers::class.java)) }
        return items
    }
}